...
iK0_final_median_cor_list = list()
Cor_fac_CCS_list = list()
Cor_fac_RT_list = list()
Cor_fac_K0_list = list()
...
for (i in 1:length(all_by_ID)) {
  ...
  #Calculate Correction for K0
  tmp = data
  tmp$CF = tmp$X1.K0 - iRT$X1.K0
  tmp$Sequence_iRT = iRT$Sequence
  tmp$CCS_iRT = iRT$X1.K0
  #substract median Correction factor from CCS
  tmp$K0_cor = tmp$X1.K0 - median(tmp$CF)
  
  iK0_final_median_cor_list [[name]] = tmp
  Cor_fac_K0_list [[name]] = median(tmp$CF)
...
iRT_K0_final_median_cor = rbindlist(iK0_final_median_cor_list)
tmp = select(iRT_RT_final_median_cor, c(CF_RT, RT_cor, RT_iRT))

# df with 1/k0 corrected CCS
# Calculate CCS from 1/K0 with mason schamp
Charge = iRT_K0_final_median_cor$charge
Mass = iRT_K0_final_median_cor$m.z * Charge
Red_mass = sqrt(1/305*(28+(Mass + Charge * 1.00727647))/(28*(Mass + Charge * 1.00727647)))

iRT_K0_final_median_cor$CCS_cor = 18500 * Charge *iRT_K0_final_median_cor$K0_cor * Red_mass
iRT_K0_final_median_cor$CCS_uncor_Mason = 18500 * Charge *iRT_K0_final_median_cor$X1.K0 * Red_mass

iRT_K0_final_median_cor = cbind(iRT_K0_final_median_cor, tmp)

iRT_final_median_cor_export = select(iRT_K0_final_median_cor, c(Sequence, ID, CCS, CCS_cor, RT, RT_cor, m.z, charge, Raw.file))
...
saveRDS(Cor_fac_K0_list, paste("Cor_fac_K0_list.rds"))
saveRDS(Cor_fac_RT_list, paste("Cor_fac_RT_list.rds"))

# For Fig.2A
saveRDS(iRT_K0_final_median_cor, paste("iRT_final_median_cor.rds"))
