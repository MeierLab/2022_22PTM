...
Calculate.mean.function.total = function(a) {
  ...
  for (i in 1:(length(a))){
    ...
    for (j in 1:length(Columns)) {
      ...
      #l = if (length(n3) == 0) {mean_3 == 0} else {
    mean_3 = Calculate.mean.function(n3, Col)
    mean_2 = Calculate.mean.function(n2, Col)
    N1 = n1[,..Col] [[1]] %>% as.list()
    ...
    names(df) = c("CCS", "Sequence", "CCS_cor", "m.z", "Retention.time", "RT_cor")
  ...
  peptide.matching.function.2 = function(a, Charge){
    ...
    for (i in 1:(length(a))){
      ...
      matched = data.frame(Sequence = matched_x$Sequence,
                           ...
                           RT_cor_mod = matched_x$Retention.time, 
                           RT_cor_unmod = matched_y$Retention.time,
                           Charge = Charge,
                           Modified.sequence = matched_x$Modified.sequence,
                           Modifications = matched_x$Modifications,
                           ID = rep(name))
